...
class InventoryListCreateView(APIView):
    ...
    def get(self, request: Request, *args, **kwargs) -> Response:
        ...
        created_at_param: str = request.query_params.get('created_at')

        if created_at_param:
            try:
                created_at: datetime = datetime.strptime(created_at_param, '%Y-%m-%d')
            except ValueError as wrong_format:
                # log the error
                return Response({'error': "created_at must be '%Y-%m-%d'"}, status=400)
